...
public class RoomCreation : MonoBehaviour
{
    ...
    public void PlaceRoom()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            ...
            if (i % 10 != 0 && i % 10 != 9)
            {
                if (roomMap.ContainsKey(i - 1) && roomMap.ContainsKey(i + 1) && roomMap[i - 1].RoomType != 0 &&
                    roomMap[i + 1].RoomType != 0 && !roomMap.ContainsKey(i - 10) && !roomMap.ContainsKey(i + 10))
                {
            ...
            if (i > 9 && i < 90)
            {
                if (roomMap.ContainsKey(i - 10) && roomMap.ContainsKey(i + 10) && roomMap[i - 10].RoomType != 0 &&
                    roomMap[i + 10].RoomType != 0 && !roomMap.ContainsKey(i - 1) && !roomMap.ContainsKey(i + 1))
                {
                    tmpGO.GetComponentInChildren<TMP_Text>().text    = "<color=#f1c94f>|</color>";
            ...
            if (!shouldCreateWall && parentDirection == 3)
                Instantiate(RoomNoDoor, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 0, 0)),
                    tmpGO.transform);
            else
                CreateWallOrDoor(tmpGO.transform, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 0, 0)),
                    shouldCreateWall);

            //우(90)
            shouldCreateWall = i % 10 > 8 || (!currentNode.Children.Contains(i + 1) && parentDirection != 1);
            if (!shouldCreateWall && parentDirection == 1)
                Instantiate(RoomNoDoor, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 90, 0)),
                    tmpGO.transform);
            else
                CreateWallOrDoor(tmpGO.transform, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 90, 0)),
                    shouldCreateWall);

            //하(180)
            shouldCreateWall = i < 10 || (!currentNode.Children.Contains(i - 10) && parentDirection != 2);
            if (!shouldCreateWall && parentDirection == 2)
                Instantiate(RoomNoDoor, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 180, 0)),
                    tmpGO.transform);
            else
                CreateWallOrDoor(tmpGO.transform, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 180, 0)),
                    shouldCreateWall);

            //좌(270)
            shouldCreateWall = i % 10 < 1 || (!currentNode.Children.Contains(i - 1) && parentDirection != 0);
            if (!shouldCreateWall && parentDirection == 0)
                Instantiate(RoomNoDoor, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 270, 0)),
                    tmpGO.transform);
            else
                CreateWallOrDoor(tmpGO.transform, tmpGO.transform.position, Quaternion.Euler(new Vector3(0, 270, 0)),
                    shouldCreateWall);
