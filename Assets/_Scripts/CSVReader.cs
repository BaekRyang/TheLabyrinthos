...
public class CSVReader : MonoBehaviour
{
    ...
    {
        Item.parsing += (item, dataValue) =>
        {
            Debug.Log("Parsing 할 데이터 없음");
        };
        Weapon.parsing += (baseItem, dataValue) =>
        {
            var    startIndex = 5;
        ...
        Disposable.parsing += (baseItem, dataValue) =>
        {
            var        startIndex = 11;
            Disposable item       = baseItem as Disposable;
            for (int i = startIndex; i < dataValue.Length; i++)
            {
                if (dataValue[i] == "" || float.Parse(dataValue[i]) == 0)
                    continue;

                switch (i - startIndex)
                {
                    case 0:
                        item.restoreHp     =  int.Parse(dataValue[i]);
                        item.s_description += "\n체력 " + item.restoreHp + " 회복";
                        
                        item.dele_itemEffect += () =>
                        {
                            var hp = Player.Instance.PS_playerStats;
                            hp.Health += item.restoreHp;
                    ...
                    case 1:
                        item.restoreHpPercent =  float.Parse(dataValue[i]);
                        item.s_description    += "\n전체체력 " + item.restoreHpPercent * 100 + "% 회복";
                    ...
                    case 2:
                        item.restoreHpLossPercent =  float.Parse(dataValue[i]);
                        item.s_description        += "\n잃은체력 " + item.restoreHpLossPercent * 100 + "% 회복";
        ...
        for (int i = 0; i < data_value.Length; i++) //각 열의 데이터를 읽어서 아이템 데이터에 저장한다.
        {
            switch (i)
            {
                ...
                default:                                                                      //아이템 공통 내용을 전부 작성했다면, 아이템 특수 내용을 작성한다. 이때 위에서 작성한 Dictionary를 사용한다.
                    ...
                    parseAttributes?.Invoke(tmpItem, data_value);                             //위 함수가 Null이 아니면 호출한다. tmpItem을 넘겨서 거기에 데이터를 저장한다.
    ...
    {
        Crafting     C_crafting = InventoryManager.Instance.craftingTab.GetComponent<Crafting>();
